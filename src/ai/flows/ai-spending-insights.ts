...
const fallbackOutput: AiSpendingInsightsOutput = {
  summary: "Não foi possível gerar uma análise detalhada neste momento, mas o teu controlo financeiro continua ativo.",
  trends: ["Continua a registar os teus gastos para veres tendências futuras."],
  savings: ["Tenta manter os teus gastos essenciais sob controlo este mês."],
  peakAnalysis: "Análise de picos temporariamente indisponível."
};

export async function getAiSpendingInsights(input: AiSpendingInsightsInput): Promise<AiSpendingInsightsOutput> {
  try {
    return await aiSpendingInsightsFlow(input);
  } catch (error) {
    console.error("Genkit AI Insights Error:", error);
