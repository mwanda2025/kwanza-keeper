...
import { generateTtsInsights } from '@/ai/flows/tts-insights-flow';
import { Button } from './ui/button';
import { Sparkles, Loader2, TrendingUp, Lightbulb, Activity, ArrowRight, AlertCircle, Volume2, VolumeX } from 'lucide-react';
...
export function AIInsightsView({ expenses, monthTotal, dailyAvg }: AIInsightsViewProps) {
  ...
  const generate = async () => {
    ...
    try {
      ...
      toast({ 
        ...
        description: "Não foi possível gerar a análise inteligente agora. Tenta mais tarde." 
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleListen = async () => {
    ...
    setTtsLoading(true);
    try {
      const fullText = `${report.summary}. As tendências encontradas são: ${report.trends.join('. ')}. Dicas de poupança: ${report.savings.join('. ')}.`;
      const { audioDataUri } = await generateTtsInsights({ text: fullText });
      setAudioUrl(audioDataUri);
      setIsPlaying(true);
    } catch (err) {
      toast({ title: "Erro de Áudio", description: "Não foi possível gerar a narração.", variant: "destructive" });
    } finally {
      setTtsLoading(false);
    }
  };
      ...
      <div className="flex items-center justify-between px-1">
        ...
        {report && (
          <Button 
            ...
            disabled={ttsLoading}
            className="h-8 gap-2 rounded-xl text-[10px] font-black uppercase tracking-widest text-primary bg-primary/5"
          >
            {ttsLoading ? <Loader2 className="h-3 w-3 animate-spin" /> : isPlaying ? <VolumeX className="h-3 w-3" /> : <Volume2 className="h-3 w-3" />}
            {isPlaying ? 'Parar Áudio' : 'Ouvir Análise'}
          </Button>
        )}
      </div>
      ...
      {report ? (
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <Card className="p-5 bg-primary/5 border-primary/20 space-y-6 rounded-2xl relative overflow-hidden">
            ...
            <Button variant="ghost" size="sm" onClick={() => { setReport(null); setIsPlaying(false); setAudioUrl(null); }} className="w-full h-8 text-[9px] font-black uppercase tracking-widest text-muted-foreground hover:text-primary mt-2">
              Limpar e Recalcular
            </Button>
        ...
        >
          {loading ? (
            ...
            <div className="flex flex-col items-center gap-3 text-destructive/60">
              <AlertCircle className="h-8 w-8" />
              <p className="text-[10px] font-black uppercase tracking-widest">IA Temporariamente Indisponível</p>
              <p className="text-[8px] uppercase">Clica para tentar novamente</p>
            </div>
          ) : (
            <div className="flex flex-col items-center gap-3">
