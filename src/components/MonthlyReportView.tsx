...
export function MonthlyReportView({ expenses, budget }: MonthlyReportViewProps) {
  ...
  const handleListen = async () => {
    ...
    setTtsLoading(true);
    try {
      const fullText = `Relatório do KwanzaKeeper. ${report.summary}. As tuas tendências: ${report.trends.join('. ')}. Sugestões: ${report.savings.join('. ')}.`;
      const { audioDataUri } = await generateTtsInsights({ text: fullText });
      setAudioUrl(audioDataUri);
      setIsPlaying(true);
    } catch (err) {
      toast({ title: "Erro de Áudio", description: "Não conseguimos gerar a narração.", variant: "destructive" });
    } finally {
      setTtsLoading(false);
    }
  };
      ...
      <div className="flex items-center justify-between px-1">
        ...
        <div className="flex gap-2">
           <Button 
            ...
            size="icon" 
            className="h-8 w-8"
            onClick={() => setSelectedMonth(prev => prev ? new Date(new Date(prev).setMonth(prev.getMonth() - 1)) : null)}
          >
        ...
        <div className="space-y-6 relative z-10">
          <div className="flex justify-between items-start">
            ...
            <div className="text-right">
               ...
               <p className="text-xs font-bold text-primary">Fecha ao Dia {settings.reportDay}</p>
            </div>
          </div>
          ...
          <Card className="p-6 bg-primary/5 border-primary/20 space-y-6 rounded-2xl">
            <div className="flex items-center justify-between">
              ...
              <Button 
                ...
                disabled={ttsLoading}
                className="h-8 gap-2 rounded-xl text-[9px] font-black uppercase tracking-widest text-primary bg-primary/10"
              >
